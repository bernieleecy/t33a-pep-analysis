# For looking at R17 to E981, E984 and E1061 distances
# Usually using rules.name_of_rule.output[0] to refer to files for plotting


rule make_t33a_glutamates_ndx:
    input:
...
rule make_t33a_e1061_xvgs:
    input:
        xtc="runs/{folder}/combined.xtc",
        ndx="runs/{folder}/glutamate_dist.ndx",
    output:
        "results/{folder}/glutamate_dists/data/e1061_r17_cz.xvg",
    params:
        prefix="runs/{folder}",
    shell:
        """
        gmx distance -f {input.xtc} -s {params.prefix}/{config[md_tpr]} \
                     -n {input.ndx} \
                     -select 'group "r_1061_&_CD" plus group "r_17_&_CZ"' \
                     -oall {output} -tu ns
        """
...
rule plot_glu_violins:
    ...
    '''
    input:
        glu_1_r17=rules.make_t33a_e981_xvgs.output[0],
        glu_2_r17=rules.make_t33a_e984_xvgs.output[0],
        glu_3_r17=rules.make_t33a_e1061_xvgs.output[0],
    output:
...
rule plot_t33a_e981_e1061_r17_heatmaps:
    """
    Make 2 heatmaps side-by-side
    Here, lig_1 and lig_2 atoms are the same (R17)
    """
    input:
        f1=rules.make_t33a_e981_xvgs.output[0],
        f2=rules.make_t33a_e1061_xvgs.output[0],
    output:
        "results/{folder}/glutamate_dists/E981_E1061_R17_heatmap.png",
    params:
        pro_1="E981",
        pro_2="E1061",
        lig_1="R17",
...
rule plot_t33a_e981_e984_r17_heatmaps:
    """
    Make 2 heatmaps side-by-side
    Here, lig_1 and lig_2 atoms are the same (R17)
    """
    input:
        f1=rules.make_t33a_e981_xvgs.output[0],
        f2=rules.make_t33a_e984_xvgs.output[0],
    output:
...
rule make_t33a_e981_xvgs_clip_10ns:
    input:
...
rule make_t33a_e984_xvgs_clip_10ns:
    input:
        xtc="runs/{folder}/combined_clip_10ns.xtc",
        ndx="runs/{folder}/glutamate_dist.ndx",
    output:
...
rule make_t33a_e1061_xvgs_clip_10ns:
    input:
        xtc="runs/{folder}/combined_clip_10ns.xtc",
        ndx="runs/{folder}/glutamate_dist.ndx",
    output:
        "results/{folder}/glutamate_dists/data/clip_e1061_r17_cz.xvg",
    params:
        prefix="runs/{folder}",
    shell:
        """
...
rule plot_glu_violins_clip_10ns:
    input:
        glu_1_r17=rules.make_t33a_e981_xvgs_clip_10ns.output[0],
        glu_2_r17=rules.make_t33a_e984_xvgs_clip_10ns.output[0],
        glu_3_r17=rules.make_t33a_e1061_xvgs_clip_10ns.output[0],
    output:
